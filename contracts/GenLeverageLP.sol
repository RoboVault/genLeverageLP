...
abstract contract GenCoreStrategy is BaseStrategy {
    ...
    {
        // TO DO ADD PENDING HARVEST LOGIC 
        if(canHarvest()){
            _harvestInternal();
        }
    ...
    function approveContracts() external virtual onlyGovernance {
        ...
        IERC20(address(shortAshortBLP)).safeApprove(address(router), uint256(-1));
    ...
    function resetApprovals() external virtual onlyGovernance {
        ...
        IERC20(address(shortAshortBLP)).safeApprove(address(router), uint256(0));
    ...
    function rebalanceDebt() external onlyKeepers {
        ...
        require(debtRatioA > debtUpper || debtRatioB > debtUpper);
    ...
    function _harvestInternal() internal returns (uint256 _wantHarvested) {
        ...
        if (maxDebt > BASIS_PRECISION){
            if (calcDebtRatioA() > calcDebtRatioB()){
                _sellHarvest(address(shortA));
                _repayDebtA();
            } else {
                _sellHarvest(address(shortB));
                _repayDebtB();                
            }
    ...
    function _sellTradingFees() internal {
        if (calcDebtRatioA() < debtLower && calcDebtRatioB() < debtLower){
            uint256 lpPercentRemove = BASIS_PRECISION.sub(Math.max(calcDebtRatioA(), calcDebtRatioB()));
            _removeLpPercent(lpPercentRemove);
            _swapExactShortAWant(balanceShortA());
            _swapExactShortBWant(balanceShortB());
        }
    }
    ...
    function _rebalanceCollateralInternal() internal {
        ...
        if (collatRatio > collatTarget) {
            uint256 percentRemoved = (collatRatio.mul(rebalancePercent).div(collatTarget)).sub(BASIS_PRECISION);
            _removeLpPercent(percentRemoved);
            _repayDebtA();
            _repayDebtB();
            emit CollatRebalance(collatRatio, percentRemoved);
        } else if (collatRatio < collatTarget) {
            uint256 percentAdded = (collatTarget.mul(rebalancePercent).div(collatTarget)).sub(BASIS_PRECISION);
            uint256 borrowAmtA = balanceShortAinLP().mul(percentAdded).div(BASIS_PRECISION);
            uint256 borrowAmtB = balanceShortBinLP().mul(percentAdded).div(BASIS_PRECISION);
            _borrowA(borrowAmtA);
            _borrowB(borrowAmtB);
            _addToLP();
            _depoistLp();

            // if any excess repay this 
            _repayDebtA();
            _repayDebtB();
    ...
    function _deploy(uint256 _amount) internal {
        ...
        uint256 borrowAmtA = _amount.mul(collatTarget).mul(1e18).div((oracleA.getPrice())).div(2);
        uint256 borrowAmtB = borrowAmtA.mul(shortB.balanceOf(shortAshortBLP)).div(shortA.balanceOf(shortAshortBLP));
        _lendWant(_amount);
        _borrowA(borrowAmtA);
        _borrowB(borrowAmtB);
        _addToLP();
    ...
    function _deployFromLend(uint256 _amount) internal {
        ...
        uint256 _borrowAmtA = balanceLend().mul(collatAdj).mul(1e18).div((oracleA.getPrice())).div(2);
        uint256 _borrowAmtB = _borrowAmtA.mul(shortB.balanceOf(shortAshortBLP)).div(shortA.balanceOf(shortAshortBLP));

        _borrowA(_borrowAmtA);
        _borrowB(_borrowAmtB);
        _addToLP();
        _depoistLp();
        _repayDebtA();
        _repayDebtB();
    ...
    function _rebalanceDebtInternal() internal {
        ...
        */
        ...
        if (debtRatioA > debtUpper) {
            ...
            swapPercent = debtInShortA.mul(rebalancePercent).div(balShortA);
            swapAmt = balShortB.mul(swapPercent).div(BASIS_PRECISION);
            _swapExactShortRebalance(address(shortB), address(shortA), swapAmt);
            _repayDebtA();
        } else {
            swapPercent = debtInShortB.mul(rebalancePercent).div(balShortB);
    ...
    function balanceDeployed() public view returns (uint256) {
        return
            balanceLend().add(balanceLp()).sub(balanceDebt());
    }

    // calculate total value of vault assets according to oracle
    function balanceDeployedOracle() public view returns (uint256) {
        return
            balanceLend().add(balanceLpOracle()).sub(balanceDebt());
    }

    // debt ratio - used to trigger rebalancing of debt
    function calcDebtRatioA() public view returns (uint256) {
        return (balanceDebtInShortA().mul(BASIS_PRECISION).div(balanceShortAinLP()));
    }

    function calcDebtRatioB() public view returns (uint256) {
        return (balanceDebtInShortB().mul(BASIS_PRECISION).div(balanceShortBinLP()));
    }
    ...
    function calcCollateral() public view returns (uint256) {
        uint256 balanceDebt = balanceDebtOracleA().add(balanceDebtOracleB());
        return
    ...
    function balanceShortAinLP() public view returns(uint256) {
        ...
        uint256 lpBalance = countLpPooled().add(shortAshortBLP.balanceOf(address(this)));
    ...
    function balanceDebt() public view returns (uint256) {
        return(balanceDebtOracleA().add(balanceDebtOracleB()));
    }
     ...
     */
    function balanceDebtOracleA() public view returns (uint256) {
        return convertShortAToWantOracle(balanceDebtInShortA());
    }

    function balanceDebtOracleB() public view returns (uint256) {
        return convertShortBToWantOracle(balanceDebtInShortB());
    }
    ...
    */
    ...
    function _swapExactShortRebalance(address _swapFrom, address _swapTo, uint256 _amountShort)
        internal
    {
        uint256 minOut = 0;
        router.swapExactTokensForTokens(
    ...
    function _swapExactShortAWant(uint256 _amountShort)
        internal
        returns (uint256 _amountWant, uint256 _slippageWant)
    {
        _amountWant = convertShortAToWantOracle(_amountShort);
        uint256[] memory amounts =
            router.swapExactTokensForTokens(
                _amountShort,
                _amountWant.mul(slippageAdj).div(BASIS_PRECISION),
                getTokenOutPath(address(shortB), address(want)),
                address(this),
                now
            );
    ...
    function _swapExactShortBWant(uint256 _amountShort)
        internal
        returns (uint256 _amountWant, uint256 _slippageWant)
    {
        _amountWant = convertShortBToWantOracle(_amountShort);
        uint256[] memory amounts =
            router.swapExactTokensForTokens(
                _amountShort,
                _amountWant.mul(slippageAdj).div(BASIS_PRECISION),
                getTokenOutPath(address(shortA), address(want)),
                address(this),
                now
            );
        _slippageWant = _amountWant.sub(amounts[amounts.length - 1]);
